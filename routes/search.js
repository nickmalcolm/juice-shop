...
module.exports = function searchProducts () {
  return (req, res, next) => {
    ...
    const orderBy = (criteria > 0) ? 'ORDER BY name' : ''
    models.sequelize.query(`SELECT * FROM Products WHERE ((name LIKE '%${criteria}%' OR description LIKE '%${criteria}%') AND deletedAt IS NULL) ${orderBy}`)
      .then(([products]) => {
